...
- Один пользователь.
- Текущий MVP использует один backend на VPS.
- Целевая конфигурация поддерживает несколько независимых backend-узлов
  одновременно: например, локальный компьютер и VPS.
- Доступ к каждому узлу выполняется через HTTPS, предпочтительно внутри
  Tailscale; LAN/SSH-туннель остаются вариантами для разработки.
- Runtime Pi исполняется на узле, где находятся его repo/worktree, secrets и
  tools. Project и созданные внутри него Chat неявно между узлами не мигрируют.
- Web-клиент поддерживает desktop/tablet/mobile и устанавливается как PWA.
...
18. Добавить несколько компьютеров, видеть их доступность и проекты в одном
    интерфейсе.
19. Одновременно работать с Chat на разных компьютерах без смешивания истории,
    событий и команд.
20. Установить desktop Web-клиент как PWA и запускать его в отдельном окне.
...
- Пользователь всегда видит, на каком компьютере находятся Project/Chat и где
  исполняется Task.
- Идентичность ресурса включает серверный узел; выбор другого узла в интерфейсе
  не может перенаправить команду существующего Chat.

Подробная целевая архитектура: `docs/13-multi-server-pwa.md`.
